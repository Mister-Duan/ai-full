...
## MAC：`pyenv global` 后找不到 `python` 命令

`pyenv global 3.14.1` 只是把默认版本写进配置，**不等于**终端已经能用上这个 Python。

常见现状：

- `pyenv global` 已是 `3.14.1`
- shim 也有：`~/.pyenv/shims/python`
- 但 `PATH` 里没有 `~/.pyenv/shims`
- `~/.zshrc` 里也没有 `eval "$(pyenv init -)"`

原因：shell 没有初始化 pyenv，shim 不在 `PATH` 中，所以 `python` 命令找不到。
...
说明：`$HOME` 是当前用户的家目录环境变量（本机一般为 `/Users/你的用户名`），所以 `"$HOME/.pyenv"` 等价于 `~/.pyenv`。
...
应分别指向 `~/.pyenv/shims/python` 和 `3.14.1`。
