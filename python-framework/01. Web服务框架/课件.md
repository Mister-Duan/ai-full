...
- `DTO (Data Transfer Object)`：用于跨层传输数据，常见于请求参数和响应结果。
- `Entity`：用于映射数据库表结构，承载持久化状态。
...
| :--- | :--- | :--- |
| 核心目标 | 数据传输 | 数据持久化 |
| 主要位置 | 接口层、应用层边界 | 领域层、基础设施层（ORM） |
| 是否直接对应数据库表 | 不一定 | 通常对应 |
| 字段设计 | 面向接口输入输出，按需裁剪 | 面向存储结构，字段更完整 |
| 敏感字段处理 | 通常不暴露 | 可能包含内部字段（如密码哈希） |
| 生命周期 | 短，常随一次请求创建和销毁 | 长，可被查询、更新、持久化 |

#### 典型配合方式

1. 前端请求先进入 `DTO`，完成格式校验。
2. 服务层将 `DTO` 转换为 `Entity` 并写入数据库。
3. 查询到 `Entity` 后，再转换为响应 `DTO` 返回前端。

#### 示例（FastAPI + SQLAlchemy 常见写法）
...
def to_entity(dto: CreateUserDTO, password_hash: str) -> UserEntity:
    return UserEntity(id=0, username=dto.username, password_hash=password_hash)


def to_response_dto(entity: UserEntity) -> UserResponseDTO:
    return UserResponseDTO(id=entity.id, username=entity.username)
```

#### 实战建议

- 不要直接把 `Entity` 原样返回给前端。
- 为“创建/更新/响应”分别定义 DTO，避免一个模型走天下。
- 当数据库结构调整时，优先保持 DTO 稳定，减少接口变更影响。
